...
function session(
  ...
  const built = WorkoutSession.open(
    "user-1",
    DateOnly.parse(date),
    { routineId: null, templateId: null, isRestDay },
    deps,
  );
...
describe("weekly volume", () => {
  ...
  it("keeps empty weeks in the series", () => {
    ...
    expect(points.map((p) => p.weekStart.value)).toEqual([
      "2026-08-10",
      "2026-08-17",
      "2026-08-24",
      "2026-08-31",
  ...
  it("distinguishes a rest day from a day with nothing recorded", () => {
    const history = TrainingHistory.of(
      [
        ...
        session("2026-09-02", [{ exerciseId: "bench", reps: 10 }]),
      ],
      [exercise("bench")],
    );

    const byDate = new Map(
      consistencyCalendar(history, 1, TODAY).map((d) => [d.date.value, d]),
    );
...
describe("muscle group balance", () => {
  ...
  it("files exercises with no muscle group under Other", () => {
    ...
    expect(muscleGroupBalance(history, 28, TODAY)).toEqual([
      { muscleGroup: OTHER_MUSCLE_GROUP, setCount: 1 },
    ]);
  });
...
describe("progress series and records", () => {
  ...
  it("keeps only the best value on a day", () => {
    ...
    expect(progressSeries(history)[0].points.map((p) => p.value)).toEqual([
      12, 9,
    ]);
  });
  ...
  it("reports each exercise's all-time best", () => {
    const history = TrainingHistory.of(
      [
        session("2026-09-01", [{ exerciseId: "pullup", reps: 12 }]),
        session("2026-09-03", [{ exerciseId: "pullup", reps: 9 }]),
      ],
      [exercise("pullup")],
    );
  ...
  it("ignores sets whose exercise it cannot resolve", () => {
    const history = TrainingHistory.of(
      [session("2026-09-03", [{ exerciseId: "ghost", reps: 10 }])],
      [],
    );
