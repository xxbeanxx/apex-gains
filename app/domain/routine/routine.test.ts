...
describe("slots", () => {
  ...
  it("reorders by swapping with a neighbour", () => {
    ...
    expect(routine.slots.map((s) => s.templateId)).toEqual([
      "push",
      "legs",
      "pull",
    ]);
...
describe("fork on write", () => {
  ...
  it("removing a slot by its sample id works through the translation", () => {
    ...
    expect(
      copy.editable.removeSlot(copy.translateChildId("slot-0"), NOW),
    ).toBe(true);
  ...
  it("does not carry activation across to the fork", () => {
    ...
    expect(sample.editableCopyFor("user-1", deps()).editable.isActive).toBe(
      false,
    );
  });
});
