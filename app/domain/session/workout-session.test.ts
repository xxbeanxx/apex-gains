...
function openSession(isRestDay = false) {
  return WorkoutSession.open(
    "user-1",
    DATE,
    { routineId: "routine-1", templateId: "template-1", isRestDay },
    deps(),
  );
}
...
describe("set numbering", () => {
  ...
  it("numbers a pyramid in the order it was performed", () => {
    ...
    expect(session.setsFor("bench").map((s) => `${s.setNumber}x${s.reps}`)).toEqual(
      ["1x12", "2x10", "3x8", "4x6"],
    );
  });
