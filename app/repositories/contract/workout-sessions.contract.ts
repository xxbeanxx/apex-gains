...
export function describeWorkoutSessionsContract(subject: ContractSubject): void {
  describe('WorkoutSessionsRepository', () => {
    ...
    describe('add', () => {
      it('opens a day and reads back what the plan said it was', async () => {
        const opened = await repositories.sessions.add(
          session({ id: ids.own, date: '2026-09-03', isRestDay: true, routineId: null, templateId: null }),
        );
    ...
    it('snapshots the plan, so a later routine change cannot rewrite the day', async () => {
      const opened = WorkoutSession.open(
        ids.athlete,
        day('2026-09-03'),
      ...
      const found = await repositories.sessions.findForDate(ids.athlete, day('2026-09-03'));
      expect(found?.toSnapshot()).toMatchObject({ routineId: null, templateId: null, isRestDay: true });
    });
  });
}
