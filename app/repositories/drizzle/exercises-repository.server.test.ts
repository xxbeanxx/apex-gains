...
describe('sampleOrOwnExercisesWhere', () => {
  it("scopes to only the user's own rows when sample data is hidden", () => {
    const { sql, params } = db.select().from(exercises).where(sampleOrOwnExercisesWhere('user-1', false)).toSQL();
  ...
  it('includes sample rows not yet forked by the user when sample data is shown', () => {
    const { sql, params } = db.select().from(exercises).where(sampleOrOwnExercisesWhere('user-1', true)).toSQL();
