...
describe('sampleOrOwnRoutinesWhere', () => {
  it('hides a sample routine once the user has forked it', () => {
    const { sql, params } = db.select().from(routines).where(sampleOrOwnRoutinesWhere('user-1', true)).toSQL();
  ...
  it('is scoped to the requesting user, not any user with a fork', () => {
    const { params } = db.select().from(routines).where(sampleOrOwnRoutinesWhere('user-2', true)).toSQL();
