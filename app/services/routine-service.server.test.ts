...
import { InMemoryRoutinesRepository } from "~/repositories/routines-repository.in-memory.server";
import { InMemoryTemplatesRepository } from "~/repositories/templates-repository.in-memory.server";
import { InMemoryUnitOfWork } from "~/repositories/unit-of-work.in-memory.server";
...
beforeEach(() => {
  ...
  service = new RoutineService(
    ...
    { ids: sequentialIds("new"), clock: fixedClock(NOW) },
  );
});
...
describe("editing a sample routine", () => {
  ...
  it("forks it into a personal copy and applies the edit there", async () => {
    ...
    expect(fork?.slots.map((s) => s.templateId)).toEqual([
      "template-push",
      null,
    ]);
  });
  ...
  it("reuses the existing fork on a second edit", async () => {
    ...
    const second = await service.reanchor(
      athlete,
      "sample-1",
      DateOnly.parse("2026-09-05"),
    );
  ...
  it("removes the slot the form named, on the fork", async () => {
    const outcome = await service.removeSlot(
      athlete,
      "sample-1",
      "sample-slot-0",
    );
...
describe("activation", () => {
  beforeEach(async () => {
    await routines.save(
      sampleRoutine({ id: "a", userId: "user-1", name: "A", slots: [] }),
    );
    await routines.save(
      sampleRoutine({ id: "b", userId: "user-1", name: "B", slots: [] }),
    );
  });
  ...
  it("activating a sample activates the fork, not the sample", async () => {
    ...
    expect((await routines.findVisible(athlete.id, "sample-1"))?.isActive).toBe(
      false,
    );
  });
});
...
describe("deleting and reverting", () => {
  ...
  it("reverting a fork removes it and names the original", async () => {
    ...
    expect((await service.list(athlete)).map((r) => r.name)).toEqual([
      "Sample PPL",
    ]);
  });
