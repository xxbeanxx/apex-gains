...
import { InMemoryExercisesRepository } from "~/repositories/exercises-repository.in-memory.server";
...
import { InMemoryWorkoutSessionsRepository } from "~/repositories/workout-sessions-repository.in-memory.server";
...
describe("removing a set", () => {
  ...
  it("reports a day with no session as not found", async () => {
    expect(
      await service.removeSet(athleteWith(), TODAY, "whatever"),
