...
/**
 * The share link for a routine: the URL to send, the QR code to point a
 ...
 * also survives the redirect a *sample* routine takes, where the link
...
export function ShareRoutineDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Share {routineName}</DialogTitle>
          <DialogDescription>
            Anyone with this link can copy the routine into their own account, along with the templates and exercises it needs.
        ...
        <div className="flex flex-col items-center gap-4">
          <div className="rounded-xl border border-border bg-white p-3">
            <QrCodeImage code={qr} label={`QR code linking to the shared routine ${routineName}`} />
          </div>
