...
function Page({
  ...
  width = "default",
  ...props
}: React.ComponentProps<"main"> & { width?: Width }) {
  return (
    <main
      id="main"
      tabIndex={-1}
...
function Section({
  ...
  return (
    <section
      className={cn("mt-(--section-gap) flex flex-col gap-4", className)}
      {...props}
    >
      {title ? (
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div className="flex flex-col gap-1">
            <Heading className="font-heading text-lg font-semibold tracking-tight">
              {title}
            </Heading>
            {description ? (
