...
export default function Home() {
  return (
    <main
      id="main"
      tabIndex={-1}
      className="mx-auto w-full max-w-(--content-max) flex-1 px-(--page-px) outline-none"
    >
      <section className="flex flex-col items-center gap-6 py-20 text-center sm:py-28">
        <span className="animate-fade-in inline-flex items-center gap-2 rounded-full bg-brand-muted px-3 py-1 text-xs font-medium text-brand-strong ring-1 ring-brand/25">
          <span
            aria-hidden="true"
            className="size-1.5 rounded-full bg-brand-strong"
          />
        ...
        <p className="animate-rise-in max-w-xl text-lg text-pretty text-muted-foreground">
          Build reusable templates, cycle them on your own schedule, and log
          every set as you lift it.
        </p>
        ...
        <div className="animate-rise-in flex flex-col items-center gap-3">
          ...
          <p className="text-xs text-muted-foreground">
            Free, and yours alone. No streaks, no leaderboards.
          </p>
        </div>
      </section>
      ...
      <section
        aria-label="Features"
        className="stagger grid gap-4 pb-24 sm:grid-cols-2 lg:grid-cols-4"
      >
